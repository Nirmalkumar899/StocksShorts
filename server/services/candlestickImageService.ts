...
// where native C++ addons like 'canvas' cannot be compiled
import { createRequire } from 'module';
let createCanvasFn: ((...args: any[]) => any) | null = null;
try {
  const require = createRequire(import.meta.url);
  const canvasModule = require('canvas');
  createCanvasFn = canvasModule.createCanvas;
} catch (e) {
  console.warn('Canvas module not available — chart image generation will be disabled');
}
