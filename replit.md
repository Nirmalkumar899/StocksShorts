...
- July 5, 2025: **MAJOR**: Implemented Google Sheets Image URL support with smart fallback - added support for column I (Image URL) in Google Sheets, system now uses provided image URLs when available and falls back to generated candlestick charts when no URL is provided, includes proper validation for http/https URLs and debug logging for image URL detection
