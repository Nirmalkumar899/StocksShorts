...
- June 29, 2025: **MAJOR**: Enhanced Perplexity API integration for 20 authentic market articles - implemented priority-based news fetching with 4 articles per priority level (fraud alerts priority 1, breakout stocks priority 2, order wins priority 3, quarterly results >20% growth priority 4, IPO updates priority 5), uses verified Indian financial sources (MoneyControl, Economic Times, Business Standard, NSE, BSE), ensures current date and previous working day only coverage with proper source attribution
