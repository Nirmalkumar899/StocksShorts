...
- June 29, 2025: **MAJOR**: Enhanced AI stock analysis with improved visual design and intelligent PE calculations - replaced OpenAI with Perplexity API for accurate financial data, implemented database storage for persistent login sessions, added sector-specific PE ratio calculations (asset management uses 4x quarterly multiplier, cyclical sectors use growth-adjusted calculations), redesigned analysis display with professional formatting and visual hierarchy, comprehensive SEBI compliance disclaimers with single prominent warning, and markdown-formatted structured output for better readability
