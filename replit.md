...
- July 5, 2025: **MAJOR**: Enhanced Connect section with smart contact buttons and streamlined interface - removed filters and shortened disclaimers to save space, added intelligent contact buttons (Call, WhatsApp, Email) that automatically enable/disable based on data availability in Google Sheets, implemented direct call functionality and WhatsApp messaging with proper Indian number formatting (+91), improved compact card design with better spacing, smart button states provide clear visual feedback about available contact methods, streamlined search interface with reduced padding, maintains fair random rotation while optimizing space usage
